...
> **Source:** https://docs.qwencloud.com/developer-guides/accuracy-tuning/video-generation

Craft better video prompts

 Copy page Scenarios:

- [Text-to-video API reference](/api-reference/video-generation/wan-text-to-video/create-task)

- [Image-to-video (first frame) API reference](/api-reference/video-generation/wan-image-to-video-first-frame/create-task)

- [Image-to-video (first and last frames) API reference](/api-reference/video-generation/wan-image-to-video-first-last-frames/create-task)

- [Reference-to-video API reference](/api-reference/video-generation/wan-reference-to-video/create-task)


## [​ ](#prompt-formulas) Prompt formulas

Here, a prompt refers to a **positive prompt** that describes the visual content and motion in a video. The more accurate and detailed the description, the higher the quality of the generated video.
### [​ ](#basic-formula) Basic formula

This formula is for users who are new to AI video generation or use it for creative inspiration. Simple and flexible prompts can produce more imaginative videos.
...
This formula is for users who have some experience with AI video generation. You can add richer and more detailed descriptions to the basic formula to enhance the video&#x27;s texture, vividness, and narrative quality.
...
### [​ ](#image-to-video-formula) Image-to-video formula

Because the image already defines the entity, scene, and style, the prompt should focus on describing the desired motion and camera movements.
...
### [​ ](#sound-formula-wan-2-5-2-6) Sound formula (Wan 2.5/2.6)

With native audio capabilities, you can add descriptions for human voices, sound effects, and background music to your prompts for enhanced sound control.

**Prompt = Entity + Scene + Motion + Sound description (voice/sound effect/background music)**


- **Sound description** — Describes the sound elements in the video and their changes over time. This guides the sound content and atmosphere to match the visual narrative precisely. This includes three main categories:
 ...
 **Voice example:** A man is doing a stand-up comedy routine. He says, "Study hard and make progress every day," in a relaxed tone, at a moderate speed, with a clear voice, in American English.**Sound effect example:** A small glass ball falls from a table onto a wooden floor, making a "thud" sound in a quiet indoor environment.**BGM example:** On a rainy night, a spooky, narrow corridor has a window at the end, with suspenseful background music. 
### [​ ](#reference-to-video-formula-wan-2-6) Reference-to-video formula (Wan 2.6)

Wan 2.6 lets you generate videos by referencing the main character in an input video. You can reference the character&#x27;s appearance, motion, and voice timbre. The character can be a person, cartoon, pet, or prop.
...
### [​ ](#multi-shot-formula-wan-2-6) Multi-shot formula (Wan 2.6)

Wan 2.6 supports generating coherent narrative videos with multiple shots. You can use prompts to precisely control the shot structure, camera position, and duration. This ensures that key elements such as the entity, scene, and atmosphere remain consistent across shots.
...
## [​ ](#prompt-optimization-techniques) Prompt optimization techniques

### [​ ](#1-automatic-optimization-enable-the-prompt-extend-parameter) 1. Automatic optimization: Enable the prompt_extend parameter

When you call the API, enable the `prompt_extend` parameter. The system automatically optimizes and rewrites your positive prompt. This feature works well for short, general prompts, such as "A kitten plays with a snowball in the snow."
 For examples showing how to use `prompt_extend` in API calls, see the [Text-to-video](/developer-guides/video-generation/text-to-video) and [Image-to-video](/developer-guides/video-generation/image-to-video) developer guides. 
### [​ ](#2-generate-prompts-using-prompt-formulas) 2. Generate prompts using prompt formulas

When you need to precisely control image details, style, or complex dynamics, you can write prompts using the structured prompt formulas.
You can provide the formulas as a system message to a large language model (LLM), such as qwen3.7-plus, to generate well-structured prompts.
 Example: Use the sound formula to optimize a prompt

 This example uses the OpenAI SDK. Before running, install the dependency:Copy ```\npip install openai

``` For more code examples, see [Overview of text generation models](/developer-guides/getting-started/text-generation-models). **Input example**Copy ```\nimport os
...
try:
 ...
 print("For more information, see the document: https://docs.qwencloud.com/api-reference/preparation/error-messages")
...
*All examples below were generated using Wan 2.2 unless noted otherwise. Sound generation examples use Wan 2.5 Preview.*
...
#### [​ ](#character-emotion) Character emotion
...
#### [​ ](#basic-camera-movement) Basic camera movement
...
Camera moves left
 ...
 A shop window on a bustling city commercial street.The camera moves left, slowly panning across the window of a luxury store, which displays glamorous models and expensive goods. The camera continues to move left, away from the window, to reveal a ragged homeless person shivering in a corner of a nearby alley. 
#### [​ ](#advanced-camera-movement) Advanced camera movement
...
#### [​ ](#special-effect-shots) Special effect shots
...
Time-lapse photography
 ...
 Time-lapse photography, dusk, sunset, rim light, medium full shot, side light, warm tones, soft light, moderate contrast, center composition. In the shot, an anthropomorphic metal robot walks on a busy city street. Its shell has a silver-gray luster, its body structure is precise, and there are fine mechanical textures at the joints. The robot is about two meters tall, with an oval head and two faintly glowing blue sensors as eyes. It swings its arms naturally, its steps are steady, and its leg joints make a slight hydraulic sound as it walks. On both sides of the street are tall glass curtain wall buildings. The setting sun&#x27;s glow falls on the buildings and the robot, creating a soft contour light. Pedestrians shuttle in the background, and the blurred crowd adds to the urban rhythm. The ground is covered with dark grey asphalt, with some areas reflecting the sky and buildings. [Previous ](/developer-guides/accuracy-tuning/image-generation)[Speech recognition best practices Optimize ASR quality through audio best practices, vocabulary customization, and post-processing correction. Next ](/developer-guides/accuracy-tuning/speech-recognition)
